...
$LOADED_FEATURES << 'legion/extensions/actors/singleton' unless $LOADED_FEATURES.include?('legion/extensions/actors/singleton')
...
RSpec.describe Legion::Extensions::Metering::Actor::Rollup do
  ...
  describe '#runner_class' do
    it 'returns the rollup runner class string' do
      expect(actor.runner_class).to eq('Legion::Extensions::Metering::Runners::Rollup')
    end
  end

  describe '#runner_function' do
    it 'returns rollup_hour' do
      expect(actor.runner_function).to eq('rollup_hour')
    end
  end

  describe '#time' do
    it 'returns 3600 (once per hour)' do
